...
void SdkSample::Initialize(Windows::UI::Xaml::Controls::TextBlock^ statusMessage, Windows::UI::Core::CoreDispatcher^ dispatcher)
{
    ...
    if (isError || isStatus) 
    {
        if (_statusMessage && _dispatcher)
        {
                ...
                ref new Windows::UI::Core::DispatchedHandler([this, message, isError]()
            {
                ...
                if (isError)
                {
                    _statusMessage->Foreground = ref new Windows::UI::Xaml::Media::SolidColorBrush(Windows::UI::Colors::Blue);
                }
                else
                {
                    _statusMessage->Foreground = ref new Windows::UI::Xaml::Media::SolidColorBrush(Windows::UI::Colors::Green);
                }
            }));
        }
    }
