...
void Scenario6::OnNavigatedTo(NavigationEventArgs^ e)
{
	...
	webViewLabel->Text = FormatStr("Webview: %s", src);
...
void Scenario6::webView6_ScriptNotify(Object^ sender, NotifyEventArgs^ e)
{
	...
	appendLog(FormatStr("Response from script at '%s': '%s'", e->CallingUri->DisplayUri,  e->Value), c);
}
...
void Scenario6::appendLog(String^ logEntry, Color c)
{
	Documents::Run^ r = ref new Documents::Run();
	r->Text = logEntry;
	Documents::Paragraph^ p = ref new Documents::Paragraph();
	p->Foreground = ref new SolidColorBrush(c);
	p->Inlines->Append(r);
