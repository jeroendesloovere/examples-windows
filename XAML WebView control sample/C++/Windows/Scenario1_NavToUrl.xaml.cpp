...
void Scenario1::WebView1_NavigationStarting(WebView^ sender, WebViewNavigationStartingEventArgs^ args)
{
	...
	address->Text = url;
	appendLog(FormatStr("Starting navigation to: \"%s\".\n", url));
	SetPageLoading(true);
...
{
	appendLog(FormatStr("Content for \"%s\" cannot be loaded into webview. Invoking the default launcher instead.\n", args->Uri->DisplayUri));
...
void Scenario1::WebView1_ContentLoading(WebView^ sender, WebViewContentLoadingEventArgs^ args)
{
	...
	appendLog(FormatStr("Loading content for \"%s\".\n", url));
}

void Scenario1::WebView1_DOMContentLoaded(WebView^ sender, WebViewDOMContentLoadedEventArgs^ args)
{
	String^ url = (args->Uri != nullptr) ? args->Uri->DisplayUri : "<null>";
	appendLog(FormatStr("Content for \"%s\" has finished loading.\n", url));
}
...
void Scenario1::WebView1_NavigationCompleted(WebView^ sender, WebViewNavigationCompletedEventArgs^ args)
{
	...
	{
		String^ url = (args->Uri != nullptr) ? args->Uri->DisplayUri : "<null>";
		appendLog(FormatStr("Navigation to \"%s\"completed successfully.\n" , url));
	}
	else
	{
		String^ url = "";
		...
		address->Text = url;
		appendLog(FormatStr("Navigation to: \"%s\" failed with error code %s.\n", url, args->WebErrorStatus.ToString())); 
	}
}

void Scenario1::appendLog(String^ logEntry)
{
	Run^ r = ref new Run();
	r->Text = logEntry;
	Paragraph^ p = ref new Paragraph();
	p->Inlines->Append(r);
	logResults->Blocks->Append(p);
}
