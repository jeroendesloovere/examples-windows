...
namespace Controls_WebView
{
    ...
    public sealed partial class Scenario1 : Page
    {
        ...
        void webView1_NavigationStarting(WebView sender, WebViewNavigationStartingEventArgs args)
        {
            ...
            {
                address.Text = url;
                rootPage.NotifyUser(String.Format("Starting navigation to: \"{0}\".\n", url), NotifyType.StatusMessage);
                pageIsLoading = true;
            }
        }
        ...
        {
            rootPage.NotifyUser(String.Format("Content for \"{0}\" cannot be loaded into webview. Invoking the default launcher instead.\n", args.Uri.ToString()), NotifyType.ErrorMessage);
        ...
        void webView1_ContentLoading(WebView sender, WebViewContentLoadingEventArgs args)
        {
            ...
            rootPage.NotifyUser(String.Format("Loading content for \"{0}\".\n", url), NotifyType.StatusMessage);
        }
        ...
        void webView1_NavigationCompleted(WebView sender, WebViewNavigationCompletedEventArgs args)
        {
            ...
            {
                string url = (args.Uri != null) ? args.Uri.ToString() : "<null>";
                rootPage.NotifyUser(String.Format("Navigation to \"{0}\"completed successfully.\n", url), NotifyType.StatusMessage);
            }
            else
            {
                string url = "";
                ...
                {
                    address.Text = url;
                    rootPage.NotifyUser(String.Format("Navigation to: \"{0}\" failed with error code {1}.\n", url, args.WebErrorStatus.ToString()), NotifyType.ErrorMessage);
                }
            }
        }
