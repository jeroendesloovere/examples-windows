...
//
// Scenario1_internetconnectionprofile.xaml.cpp
// Implementation of the InternetConnectionProfile class
//

#include "pch.h"
#include "Scenario1_internetconnectionprofile.xaml.h"
...
InternetConnectionProfile::InternetConnectionProfile()
{
    InitializeComponent();
}
...
void InternetConnectionProfile::OnNavigatedTo(NavigationEventArgs^ e)
{
...
String^ InternetConnectionProfile::GetCostBasedSuggestions(ConnectionCost^ connectionCost)
{
    String^ strRet = "";
...
String^ InternetConnectionProfile::GetConnectionCostInfo(ConnectionCost^ connectionCost)
{
    String^ strRet = "";
...
String^ InternetConnectionProfile::GetDataPlanStatusInfo(DataPlanStatus^ dataPlan)
{
    String ^strRet = "";
...
String^ InternetConnectionProfile::GetNetworkSecuritySettingsInfo(NetworkSecuritySettings^ netSecuritySettings)
{
...
String^ InternetConnectionProfile::GetWlanConnectionProfileDetailsInfo(Windows::Networking::Connectivity::WlanConnectionProfileDetails^ wlanConnectionProfileDetails)
{
    String^ wlanDetails = "";
...
String^ InternetConnectionProfile::GetWwanConnectionProfileDetailsInfo(Windows::Networking::Connectivity::WwanConnectionProfileDetails^ wwanConnectionProfileDetails)
{
    String^ wwanDetails = "";
...
String^ InternetConnectionProfile::GetConnectionProfile(ConnectionProfile^ connectionProfile)
{
...
// Display Internet Connection Profile information
void SDKSample::NetworkInformationApi::InternetConnectionProfile::InternetConnectionProfile_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    try
    {
        //Display Internet Connection Profile information to the user
        ConnectionProfile^ InternetConnectionProfile = NetworkInformation::GetInternetConnectionProfile();
        if (InternetConnectionProfile != nullptr)
        {
            String^ strConnectionProfileInfo = GetConnectionProfile(InternetConnectionProfile);
            OutputText->Text = L"Profile Name : " + InternetConnectionProfile->ProfileName + L"\n" + strConnectionProfileInfo;
            rootPage->NotifyUser(L"Success", NotifyType::StatusMessage);
        }
        else
        {
            rootPage->NotifyUser(L"Internet Connection Profile Information not found", NotifyType::StatusMessage);
        }
    }
