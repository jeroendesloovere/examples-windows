...
// Scenario3_findconnectionprofiles.xaml.cpp
// Implementation of the FindConnectionProfiles class

#include "pch.h"
#include "Scenario3_findconnectionprofiles.xaml.h"
...
FindConnectionProfiles::FindConnectionProfiles()
{
    InitializeComponent();
}
...
void FindConnectionProfiles::OnNavigatedTo(NavigationEventArgs^ e)
{
...
String^ FindConnectionProfiles::GetCostBasedSuggestions(ConnectionCost^ connectionCost)
{
    String^ strRet = "";
    ...
    else if (connectionCost->NetworkCostType == NetworkCostType::Fixed)
    {
        ...
        {
            strRet += L"Application can implement the Conservative behavior\n";
        }
    }
    else
    {
...
String^ FindConnectionProfiles::GetConnectionCostInfo(ConnectionCost^ connectionCost)
{
    String^ strRet = "";
...
String^ FindConnectionProfiles::GetDataPlanStatusInfo(DataPlanStatus^ dataPlan)
{
    String ^strRet = "";
...
String^ FindConnectionProfiles::GetNetworkSecuritySettingsInfo(NetworkSecuritySettings^ netSecuritySettings)
{
    ...
    switch (netSecuritySettings->NetworkAuthenticationType)
    {
        ...
        case NetworkAuthenticationType::Ihv:
            networkSecurity += L"NetworkAuthenticationType: Ihv";
            break;
        default:
    ...
    switch (netSecuritySettings->NetworkEncryptionType)
    {
        ...
        case NetworkEncryptionType::Ihv:
            networkSecurity += L"NetworkEncryptionType: Ihv";
            break;
        default:
...
String^ FindConnectionProfiles::GetWlanConnectionProfileDetailsInfo(Windows::Networking::Connectivity::WlanConnectionProfileDetails^ wlanConnectionProfileDetails)
{
    String^ wlanDetails = "";
...
String^ FindConnectionProfiles::GetWwanConnectionProfileDetailsInfo(Windows::Networking::Connectivity::WwanConnectionProfileDetails^ wwanConnectionProfileDetails)
{
    String^ wwanDetails = "";
...
String^ FindConnectionProfiles::GetConnectionProfile(ConnectionProfile^ connectionProfile)
{
    String^ strProfileInfo = "";

    if (connectionProfile == nullptr)
    {
        strProfileInfo += "ConnectionProfile is null\n";
...
void FindConnectionProfiles::PrintConnectionProfiles(IVectorView<ConnectionProfile^>^ connectionProfiles)
{
    try
    {
...
// Get the Content of the comboBox as a String
String^ FindConnectionProfiles::GetComboBoxSelectedText(ComboBox^ comboBox)
{
...
void SDKSample::NetworkInformationApi::FindConnectionProfiles::FindConnectionProfilesButton_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    Guid serviceProviderGuid;
    ConnectionProfileFilter^ filter = ref new ConnectionProfileFilter();

    // Add values to the filter. If we don't care about a particular field, we don't set it.

    // IsConnected
    if (GetComboBoxSelectedText(IsConnectedComboBox) == "Yes")
    {
        filter->IsConnected = true;
    }
    else if (GetComboBoxSelectedText(IsConnectedComboBox) == "No")
    {
        filter->IsConnected = false;
    }

    // IsWlanConnectionProfile
    if (GetComboBoxSelectedText(IsWlanProfileComboBox) == "Yes")
    {
        filter->IsWlanConnectionProfile = true;
    }
    else if (GetComboBoxSelectedText(IsWlanProfileComboBox) == "No")
    {
        filter->IsWlanConnectionProfile = false;
    }

    // IsWwanConnectionProfile
    if (GetComboBoxSelectedText(IsWwanProfileComboBox) == "Yes")
    {
        filter->IsWwanConnectionProfile = true;
    }
    else if (GetComboBoxSelectedText(IsWwanProfileComboBox) == "No")
    {
        filter->IsWwanConnectionProfile = false;
    }

    // NetworkCostType
    if (GetComboBoxSelectedText(NetworkCostTypeComboBox) == "Unrestricted")
    {
        filter->NetworkCostType = NetworkCostType::Unrestricted;
    }
    else if (GetComboBoxSelectedText(NetworkCostTypeComboBox) == "Fixed")
    {
        filter->NetworkCostType = NetworkCostType::Fixed;
    }
    else if (GetComboBoxSelectedText(NetworkCostTypeComboBox) == "Variable")
    {
        filter->NetworkCostType = NetworkCostType::Variable;
    }
    else if (GetComboBoxSelectedText(NetworkCostTypeComboBox) == "Unknown")
    {
        filter->NetworkCostType = NetworkCostType::Unknown;
    }

    // Parse the GUID from the textbox
    if (!ServiceProviderGuidTextBox->Text->IsEmpty())
    {
        GUID guid;
        String^ guidString = ServiceProviderGuidTextBox->Text;
        HRESULT hr = CLSIDFromString(guidString->Data(), &guid);
        if (SUCCEEDED(hr))
        {
            filter->ServiceProviderGuid = Guid(guid);
        }
        else
        {
            rootPage->NotifyUser("Guid must be of the form: {00000000-0000-0000-0000-000000000000}", NotifyType::ErrorMessage);
            return;
        }
    }

    create_task(NetworkInformation::FindConnectionProfilesAsync(filter)).then([this](task<IVectorView<ConnectionProfile^>^> asyncInfo)
    {
        try
        {
            IVectorView<ConnectionProfile^>^ connectionProfiles = asyncInfo.get();
