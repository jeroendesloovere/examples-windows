...
#include "pch.h"
#include "Scenario2_oAuthGoogle.xaml.h"
...
#if WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP
void SDKSample::WebAuthentication::Scenario2::ContinueWebAuthentication(WebAuthenticationBrokerContinuationEventArgs^ args)
{
	...
	{
		GoogleReturnedToken->Text += "HTTP Error returned by AuthenticateAsync() : " + result->ResponseErrorDetail;
	}
	else
	{
		String^ statusString;
		...
		case WebAuthenticationStatus::Success:
			statusString = "Success";
			GoogleDebugArea->Text += result->ResponseData;
			break;
		case WebAuthenticationStatus::UserCancel:
			statusString = "UserCancel";
			break;
		}
		GoogleDebugArea->Text += "Error returned by AuthenticateAsync() : " + statusString;
	}
