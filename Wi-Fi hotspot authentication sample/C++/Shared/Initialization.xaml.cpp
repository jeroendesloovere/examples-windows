...
void Initialization::RegisterButton_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    try
    {
		...
		statusOperation->Completed = ref new AsyncOperationCompletedHandler<BackgroundAccessStatus>([this](IAsyncOperation<BackgroundAccessStatus>^ operation, AsyncStatus status)
		{
			auto backgroundStatus = operation->GetResults();
