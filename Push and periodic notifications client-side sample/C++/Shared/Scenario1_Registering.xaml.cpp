...
void SDKSample::SDKTemplate::Scenario1::OpenChannel_Click(Platform::Object^ sender, RoutedEventArgs^ e)
{
	create_task(rootPage->Notifier->OpenChannelAndUploadAsync(ServerText->Text)).then([this](task<ChannelAndWebResponse^> channelAndResponseTask)
	{
		try
		{
			auto channelAndResponse = channelAndResponseTask.get();
		...
		{
			rootPage->NotifyUser("An exception occurred while uploading channel: " + ex->Message, NotifyType::ErrorMessage);
		}
	});
}

void SDKSample::SDKTemplate::Scenario1::CloseChannel_Click(Platform::Object^ sender, RoutedEventArgs^ e)
{
