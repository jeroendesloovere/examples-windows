...
//
// Scenario1_NetworkStatusWithInternetPresent.xaml.cpp
// Implementation of the NetworkStatusWithInternetPresent class
//

#include "pch.h"
#include "Scenario1_NetworkStatusWithInternetPresent.xaml.h"
...
void NetworkStatusWithInternetPresent::OnCompleted(BackgroundTaskRegistration^ task, BackgroundTaskCompletedEventArgs^ args)
{
    auto uiDelegate = [this]()
    {
        ...
        String^ hasNewConnectionCost = safe_cast<String^>(localSettings->Values->Lookup("HasNewConnectionCost"));
        String^ hasNewDomainConnectivityLevel = safe_cast<String^>(localSettings->Values->Lookup("HasNewDomainConnectivityLevel"));
        String^ hasNewHostNameList = safe_cast<String^>(localSettings->Values->Lookup("HasNewHostNameList"));
        String^ hasNewInternetConnectionProfile = safe_cast<String^>(localSettings->Values->Lookup("HasNewInternetConnectionProfile"));
        String^ hasNewNetworkConnectivityLevel = safe_cast<String^>(localSettings->Values->Lookup("HasNewNetworkConnectivityLevel"));
            ...
            !(adapter->Equals(networkAdapter)))
        {
            ...
            String^ outputText = "Internet Profile changed\n" + "=================\n" + "Current Internet Profile : " + internetProfile + "\n\n";

            if (hasNewConnectionCost != "")
            {
                outputText += hasNewConnectionCost + "\n";
            }
            if (hasNewDomainConnectivityLevel != "")
            {
                outputText += hasNewDomainConnectivityLevel + "\n";
            }
            if (hasNewHostNameList != "")
            {
                outputText += hasNewHostNameList + "\n";
            }
            if (hasNewInternetConnectionProfile != "")
            {
                outputText += hasNewInternetConnectionProfile + "\n";
            }
            if (hasNewNetworkConnectivityLevel != "")
            {
                outputText += hasNewNetworkConnectivityLevel + "\n";
            }
