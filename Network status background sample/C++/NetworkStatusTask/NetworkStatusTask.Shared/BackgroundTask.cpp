...
void BackgroundTask::Run(IBackgroundTaskInstance^ taskInstance)
{
    ...
    try
    {
        NetworkStateChangeEventDetails^ details = (NetworkStateChangeEventDetails^)taskInstance->TriggerDetails;

        localSettings->Values->Insert("HasNewConnectionCost", details->HasNewConnectionCost ? "New Connection Cost" : "");
        localSettings->Values->Insert("HasNewDomainConnectivityLevel", details->HasNewDomainConnectivityLevel ? "New Domain Connectivity Level" : "");
        localSettings->Values->Insert("HasNewHostNameList", details->HasNewHostNameList ? "New HostName List" : "");
        localSettings->Values->Insert("HasNewInternetConnectionProfile", details->HasNewInternetConnectionProfile ? "New Internet Connection Profile" : "");
        localSettings->Values->Insert("HasNewNetworkConnectivityLevel", details->HasNewNetworkConnectivityLevel ? "New Network Connectivity Level" : "");
