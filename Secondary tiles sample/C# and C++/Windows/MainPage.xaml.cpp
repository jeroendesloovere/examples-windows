...
{    
    ...
    if (SuspensionManager::SessionState()->HasKey("SelectedScenarioIndex") && e->Parameter == nullptr)
    {
        startingScenarioIndex = safe_cast<int>(SuspensionManager::SessionState()->Lookup("SelectedScenarioIndex"));
    }
    else if (e->Parameter != nullptr)
    {
        this->LaunchParam = e->Parameter->ToString();
        startingScenarioIndex = this->GetScenarioIdForLaunch(e->Parameter->ToString());
    }
